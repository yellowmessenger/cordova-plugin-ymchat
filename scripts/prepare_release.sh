...
# established convention: bump package.json's version, plugin.xml's version
# attribute (Cordova requires the two to match), optionally the pinned
# native-SDK dependency (Android and/or iOS) if this release exists to pick
# up a native SDK change, prepend a Keep-a-Changelog-style CHANGELOG.md
...
# --android-dep only touches src/android/build.gradle's YMChatbot-Android
# pin; --ios-dep only touches plugin.xml's inline <pod name="YMChat"> spec.
# Omit whichever platform's native SDK didn't change for this release.
#
# Example:
...
ANDROID_GRADLE="src/android/build.gradle"
...
FILES_TO_COMMIT=("$PACKAGE_JSON" "$PLUGIN_XML" "$CHANGELOG")
...
# --- 2. Bump plugin.xml's version attribute (must match package.json) ---
sed -i.bak "s/<plugin id=\"cordova-plugin-ymchat\" version=\"$CURRENT_VERSION\"/<plugin id=\"cordova-plugin-ymchat\" version=\"$NEW_VERSION\"/" "$PLUGIN_XML" && rm -f "$PLUGIN_XML.bak"
if ! grep -q "<plugin id=\"cordova-plugin-ymchat\" version=\"$NEW_VERSION\"" "$PLUGIN_XML"; then
  echo "error: failed to update the version attribute in $PLUGIN_XML" >&2
  exit 1
fi

# --- 3. Optionally bump the pinned native-SDK dependencies ---
...
if [[ -n "$IOS_DEP" ]]; then
  CURRENT_IOS_DEP="$(sed -n 's/.*<pod name="YMChat" spec="~> \([^"]*\)".*/\1/p' "$PLUGIN_XML")"
  sed -i.bak "s/<pod name=\"YMChat\" spec=\"~> $CURRENT_IOS_DEP\"/<pod name=\"YMChat\" spec=\"~> $IOS_DEP\"/" "$PLUGIN_XML" && rm -f "$PLUGIN_XML.bak"
  if ! grep -q "<pod name=\"YMChat\" spec=\"~> $IOS_DEP\"" "$PLUGIN_XML"; then
    echo "error: failed to update the YMChat pod pin in $PLUGIN_XML" >&2
    exit 1
  fi
fi

# --- 4. Prepend CHANGELOG entry (Keep a Changelog style: no date, no link) ---
ENTRY_FILE="$(mktemp)"
{
  echo "## [${NEW_VERSION}]"
  echo ""
